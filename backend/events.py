...
def build_cognition_event(
    *, message_id: str, ts: float, user_msg: str, response: str,
    trackers: dict, features: list, model: str = "gemma-2-2b-it", layer: int = 12,
) -> CognitionEvent:
    ...
    return CognitionEvent(
        message_id=message_id, ts=ts, model=model, layer=layer,
